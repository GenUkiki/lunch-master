...
async function create(req, res) {
  ...
  try {
    const created = await jsonDbClient.post('/delivery_schedule', { date, status, notes });
    res.json(created);
  } catch (e) {
...
async function printDailyOrderList(req, res) {
  ...
  try {
    ...
    // CSV 風の印刷データを生成
    let csv = 'order_id,user_id,date,menu_item_id,price,status,picked_up,picked_up_at\n';
