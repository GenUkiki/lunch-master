...
function isInInputWindow() {
  ...
  return mins >= (INPUT_WINDOW_MIN_START || 0) && mins <= (INPUT_WINDOW_MIN_END || 0);
}
...
async function createOrder(req, res) {
  if (!isInInputWindow()) {
    return res.status(403).json({ error: 'Outside input window (08:30-08:50)' });
  }
  ...
  try {
    ...
    await jsonDbClient.post('/audit_logs', {
...
async function updateOrder(req, res) {
  ...
  try {
    ...
    if (existing.status === 'locked') {
      if (updates.hasOwnProperty('menu_item_id') || updates.hasOwnProperty('date') ||
          updates.hasOwnProperty('price') || updates.hasOwnProperty('status')) {
    ...
    const updated = await jsonDbClient.patch('/orders/' + orderId, { ...existing, ...updates }, siteId);
